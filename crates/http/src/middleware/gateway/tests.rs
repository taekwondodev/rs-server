...
use domain_auth::AccessTokenClaims;
use uuid::Uuid;

use crate::middleware::gateway::{inject_identity_headers, strip_forwarded_headers};
...
fn inject_sets_x_user_id_from_sub() {
    ...
    assert_eq!(req.headers().get(X_USER_ID).unwrap().to_str().unwrap(), sub.to_string());
}
...
fn inject_sets_x_user_role_when_present() {
    ...
    assert_eq!(req.headers().get(X_USER_ROLE).unwrap().to_str().unwrap(), "admin");
}
...
fn inject_overwrites_residual_role_with_real_role() {
    ...
    assert_eq!(req.headers().get(X_USER_ROLE).unwrap().to_str().unwrap(), "user");
}
...
fn inject_overwrites_residual_user_id_with_real_sub() {
    ...
    assert_eq!(req.headers().get(X_USER_ID).unwrap().to_str().unwrap(), real_sub.to_string());
}
